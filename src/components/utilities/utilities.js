...
const customTooltips = function(tooltip) {
  ...
  // Escape HTML so user-controlled chart labels (agency name, listing title, etc.)
  // cannot inject markup when written via innerHTML below.
  ...
  if (tooltip.body) {
    ...
    titleLines.forEach(function(title) {
      innerHtml += `<div class='tooltip-title'>${escapeHtml(title)}</div>`;
    });
    ...
    bodyLines.forEach(function(body, i) {
      ...
      const bodyText = (Array.isArray(body) ? body : [body]).map(escapeHtml).join(',');
      innerHtml += `<tr><td>${span}${bodyText}</td></tr>`;
    });
