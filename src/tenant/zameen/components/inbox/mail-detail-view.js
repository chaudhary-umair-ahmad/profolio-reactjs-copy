...
import { useLazyGetMessageDetailQuery, usePostReplyMutation } from '../../../../apis/lms';

const Single = (props) => {
  const { t, i18n } = useTranslation();
  const navigate = useRouteNavigate();
  const { canReply, inboxFor, type, user, fetchCount, match } = props;
  ...
  const queryIndex = Object.keys(match)[0];
  const messageId = match[queryIndex].split('/')[1];
  const [getMessageDetail, { isLoading: loading, error: error }] = useLazyGetMessageDetailQuery();
  ...
  const fetchMessage = async () => {
    ...
    if (response) {
      response?.data?.conversation && setEmail(response?.data?.conversation);
      fetchCount();
    }
  };

  const sendReply = async (replyMessage) => {
    const response = await postReply({ message: replyMessage, messageId: messageId });
    if (response) {
      if (response.error) {
        ...
        setShowReply(false);
        setReply(null);
        setEmail(response?.data?.conversation);
      }
    }
  };
